...
public class DeleteSupplierDetail {
    private static final Logger LOGGER = Logger.getLogger(DeleteBill.class.getName());

    public void sendDeleteSupplierDetailRequest(SupplierDetailDTO supplierDetailDTO) {
        try {

            Long id = supplierDetailDTO.getIdDetalleProveedor();
            ...
            HttpRequest request = HttpRequest.newBuilder()
                    .uri(URI.create("http://localhost:8086/api/supplierDetail/delete/" + id))
                    .DELETE()
                    .build();
